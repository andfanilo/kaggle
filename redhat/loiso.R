...
# Source : https://www.kaggle.com/loisso/predicting-red-hat-business-value/lb-0-987-group-1-and-date-trick
...
## if want to export loiso file only
#predictions[is.na(predictions$filled), filled := predictions[,mean(filled, na.rm = T)]]
#colnames(predictions)[2] <- "outcome"
#write.csv(predictions,"data/loiso_submission.csv", row.names = FALSE)


## if want to merge
predictions[predictions$filled == 0.5, "filled"] <- NA
write.csv(predictions,"data/loiso_submission.csv", row.names = FALSE)
